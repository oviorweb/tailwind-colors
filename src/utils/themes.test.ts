...
describe("generateCombinedColors", () => {
  it("should generate combined colors correctly", () => {
    ...
    expect(combinedColors.primary[100](opacityConfig)).toBe(
      "rgb(var(--color-primary-100) / 0.5)"
    );
    expect(combinedColors.secondary[300](opacityConfig)).toBe(
      "rgb(var(--color-secondary-300) / 0.5)"
    );
  });
});
